---
title: Linux Providing
short_name: Linux Providing
description: A simple guide on how to provide your internet to URnetwork for Linux.
---

import Callout from "@/components/Alerts/Callout";

<Callout type="danger">
    PAGE NOT FINISHED: TEMPORARY PLACEHOLDER PAGE. INSTRUCTIONS MAY BE WRONG!!!
</Callout>

# Providing to URnetwork

To provide your internet to URnetwork, you run a small provider application
on your system, which shares your internet with the network.  This application
is often referred to as the "provider binary" or "provider executable".

We maintain URnetwork provider binaries for many platforms and architectures,
including GNU/Linux, macOS and Windows.  We'll give you a short guide on how
set them up on your system.

## GNU/Linux
<Callout type="warning">
The installation script requires you have the following installed:

 * [cURL](https://curl.se)
 * Either [jq](https://jqlang.org) or [Python 3.8+](https://python.org)
 * A POSIX-compatible shell

Follow this link then copy and paste it inside of your terminal then hit enter (You must be root): 

[Install Helper](https://raw.githubusercontent.com/urnetwork/connect/refs/heads/main/scripts/Provider_Install_Deps.sh).

Or just install using your chosen package manager.
</Callout>

Run the following command to install provider for linux as well as urnet-tools:

```bash
curl -fSsL https://raw.githubusercontent.com/urnetwork/connect/refs/heads/main/scripts/Provider_Install_Linux.sh | sh
```

This command will install the latest URnetwork provider binary, and a small
helper tool called **urnet-tools**.

Most modern GNU/Linux systems already have these installed, so you usually
won't have to worry about them.  But if yours does not have these, or if you
use a very minimal system, you will have to consult your Distro's packaging
system to figure out how to install them.

After you run this script, you should follow the steps in the [post install instructions section](#post-install-instructions).


### Post-install instructions

The first thing to do after installation is to re-source your `~/.bashrc`. To do so, run:

```bash
source ~/.bashrc
```

This will ensure urnetwork commands work from now on.

<Callout type="info">
    If you do not use bash as your default shell, the installer will create a `~/.bashrc` file,
    which you should open up and copy paste its contents into your shell's profile or `*rc` file accordingly.
</Callout>

To further verify that your installation was successful, run:

```bash
urnetwork --help
urnet-tools --help
```

Both of these commands should print out help instructions.

### Authentication

Next, you have to log into URnetwork. To do so, run:

```bash
urnetwork auth
```

Then:

It will ask for an auth code. You can obtain an auth code at https://ur.io, https://app.ur.network or inside the mobile app.
The auth code button should be located in the settings section of their respective app.

<img src="https://raw.githubusercontent.com/virtual-designer/urnetwork-docs/refs/heads/main/app/(content)/urio_auth.png" height="335px" width="576px" />
<img src="https://raw.githubusercontent.com/virtual-designer/urnetwork-docs/refs/heads/main/app/(content)/web_app_auth.png" height="1199px" width="2348px" />
<img src="https://raw.githubusercontent.com/virtual-designer/urnetwork-docs/refs/heads/main/app/(content)/mobile_auth.png" height="907px" width="1179px" />


If your auth code was correctly copied, you should be logged into URnetwork now.  The command above saves your account
JWT token to `~/.urnetwork/jwt`. You should not share this with anyone.

### Starting the provider

Finally, you can start the provider:

```bash
urnet-tools start
```

Or if you want to run it in the foreground for your own purposes

```bash
urnetwork provide
```

Keep in mind if you close the foreground process it will die, so it's recommeneded to use urnet-tools to create a service automatically.

Next, it's also recommended to allow to service to startup when you restart your system so you won't have to worry about manually starting it.

Use the following to enable enable on system startup:

```bash
urnet-tools auto-start on
```

This will enable the service, use `off` to turn startup off.

If everything was set up correctly, this will not produce any output. To see the status of
the service, run:

```bash
urnet-tools status
```

This should show that the service is active and running. Congratulations, you are now a part of
URnetwork!

### Updating

If you'd like to manually update you can easily run:

```bash
urnet-tools update
```

This command also allows you to choose a build version, you can find the build tag [here](https://github.com/urnetwork/connect/releases).

If you prefer to not worry about it, you can set it to automatically update to the latest stable using:

```bash
urnet-tools auto-update on
```

This will check for updates daily, use `off` to turn auto updates off.

### Uninstalling

In case if you want to uninstall URnetwork, you can use `urnet-tools` for that:

```bash
urnet-tools uninstall
```